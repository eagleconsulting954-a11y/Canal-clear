...
async function sendLimitEmail(user, status, day) {
  ...
  if (day === 1) {
    ...
    body = `You have ${status.vesselCount} vessels on your ${tierName} plan (limit: ${limit}). You have 7 days to upgrade to ${upgradeName} before new filings are blocked. Upgrade at https://canal-clear.polsia.app/pricing`;
    html = `
<p>Hi there,</p>
...
<p><a href="https://canal-clear.polsia.app/pricing" style="background:#E87722;color:#fff;padding:12px 24px;text-decoration:none;border-radius:6px;display:inline-block;font-weight:600;">Upgrade to ${upgradeName}</a></p>
<p>Questions? Reply to this email.</p>`;
  } else if (day === 5) {
    subject = `2 days left: Upgrade your ${tierName} plan to avoid filing disruption`;
    body = `Reminder: You have 2 days to upgrade before new filings for vessels over your ${tierName} limit (${limit} vessels) are blocked. You currently have ${status.vesselCount} vessels. Upgrade at https://canal-clear.polsia.app/pricing`;
    html = `
<p>Hi there,</p>
...
<p><a href="https://canal-clear.polsia.app/pricing" style="background:#E87722;color:#fff;padding:12px 24px;text-decoration:none;border-radius:6px;display:inline-block;font-weight:600;">Upgrade Now</a></p>`;
  } else if (day === 7) {
    subject = `Filing paused: Upgrade your ${tierName} plan to resume`;
    body = `New filings are now blocked for vessels beyond your ${tierName} limit (${limit} vessels). You have ${status.vesselCount} vessels. Upgrade to resume filing. Existing vessel data and history is fully accessible. Upgrade at https://canal-clear.polsia.app/pricing`;
    html = `
<p>Hi there,</p>
